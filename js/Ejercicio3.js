//Estilizacion de la barras zona de pruebas en el DevTools en el navegador
console.log("%cEjercicio 03: Funciones ", style_console);
//Funciones nombradas
//1. Funciones procedurale, vacias, VOID, sin valor de retorno y sin parametros de entrada
function saludar()
    {
        console.log("Bienvendio al sistema de Bienes Raices");
    }
    console.warn("1. Funciones sin valor de retorno, sin parámetroso");

    saludar();

    /**
     * 2. Funciones procedurales sin valor de retorno, pero que si recibe
     * parámetros es decir datos de entrda
     * */
    ...
    //Imprimiendo el valor y que tipo de dato 
    ...
    console.log(`${loginStatus? 'El usuario admin se ha logueado satisfactoriamente':'Hubo un error en el login del usuario'}`);
    ...
    //Test 4 - Usuario y Contraseña correctos
     console.log("Test 4: - usuario: natalia, password:hola")
     loginStatus=login("natalia", "hola");
    console.log(`${loginStatus? 'El usuario admin se ha logueado satisfactoriamente':'Hubo un error en el login del usuario'}`);

    //FUNCIONES ANÓNIMAS, FUNCIONES FLECHA (ARROW), FUNCIONES LAMDA
    /**
     * A diferencia de las funciones nombradas,este tipo de funciones no suelen
     * ser urilizadas en el codigo solo se ejeciutan una sola vez, dando velocidad
     * indivividalidad y privacidad a los datos utilizados en el proceso
     */

    //5. Función an´ponima sin parametros
    isNewUser=function(){
        const hoy=new Date();
        return(
...
=======================================================================`);
    ...
    recoverPassword("natalia@gmail.com", function(systemResponse){
